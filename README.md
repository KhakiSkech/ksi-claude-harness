...
| · 스킬 4 | `skills/` | `deep-interview`(의도 확정 — UI면 UX 축·멀티액터면 **어뷰징 축** 포함) · `brainstorm`(발산→수렴) · `codebase-audit`(백엔드 병렬 감사 + 수렴 루프 + **어뷰징·무결성 불변식** 렌즈: self-dealing·경제무결성·게이밍·시간축권한) · `ui-audit`(프론트 **시각 + UX 플로우** 감사: 흐름 단계수·에러복구·용어 SSOT·마이크로카피) |
