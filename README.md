...
빠른 실험·PoC에는 검증 관문이 과할 수 있습니다. 환경변수 하나로 조절합니다:

| `KSI_HOOKS=` | 의미 | 추천 상황 |
|---|---|---|
| `strict` (기본) | 완료 관문 차단 + 모든 알림 | 배포 준비·장기 자율 실행 |
| `warn` | 완료 관문은 통과시키고, 로컬 손실 명령은 경고만 | **평상시 개발 (균형)** |
| `off` | 검증·알림 훅 침묵 | 버리는 실험·빠른 PoC |

```bash
export KSI_HOOKS=warn   # 세션 임시
# 또는 프로젝트 .claude/settings.json:  "env": { "KSI_HOOKS": "warn" }
```

⚠️ **안전벨트는 모드와 무관하게 항상 유지됩니다** — 루트 삭제·force push·DB 삭제·시크릿 push는 `off`에서도 차단됩니다. 이 스위치는 "귀찮음"을 조절하는 것이지 "안전"을 끄는 게 아닙니다.
...
| 에이전트 | 모델 | 역할 |
|---|---|---|
| `scout` | Haiku (저렴) | 탐색·집계·잡일 (코드 수정은 금지) |
...
| 완료 시도 | `ui-render-check` | 화면 변경 시 렌더 확인 요구 | ↩ 관문 |
| | `backend-verify-check` | 백엔드 변경 시 실제 동작 확인 요구 | ↩ 관문 |
...
flowchart TB
    ...
    DEL --> AG["에이전트 3<br/>scout 잡일<br/>worker 구현<br/>reviewer 검증"]
...
| **검증 게이트** | "완료" 선언 전에 실제 증거(테스트 실행·렌더 확인)를 요구하는 관문 |
