...
| **자율·검증 워크플로** | `templates/workflows/` | `audit-loop.js`(감사 루프) · `paired-run.js`(tier 검증) · `goals-run.js`(원장 자율 소진 — red-lane 하드스톱·evidence-gate 종료) · `reviewer-calibration.js`(reviewer 러버스탬프 퇴화 능동 측정 — trap-set ground-truth 채점) |
